...
export class ShowMeetDetailsComponent implements OnInit {
  ...
  deletedMeet() {
    console.log(this.meetData[0].MeetDocId)
       
    // if(this.MeetToDelete != null) {
      // delete this.meetData[0].MeetDocId ;
      console.log("check")
    const uid = this.authService.getLoggedInUser();
    const orgDomain = this.backendService.getOrganizationDomain();
       this.teamName =  this.startService.teamName;
       console.log(orgDomain,this.teamName,this.meetData[0].MeetDocId)
    const callable = this.functions.httpsCallable("meet/deleteMeet");
      callable({OrgDomain: orgDomain, TeamName: this.teamName, Id: this.meetData[0].MeetDocId}).subscribe({
        next(data) {
          // console.log(data);
          console.log("dwjmd")
         delete this.getMeetDetails;
          
          // this. getMeetData();
          // console.log(this.getMeetData());
