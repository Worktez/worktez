...
exports.setPayment = function(amount, userUid, subscriptionId, paymentId, userEmailAddress, paymentTime, paymentDate) {
    const addPaymentPromise = db.collection("Subscriptions").doc(subscriptionId).collection("Payment").doc(paymentId).set({
...
exports.generateBase62Constant = function() {
  ...
  * @return {any}
  */
exports.setRazorDetails = function(paymentId, order, subscriptionId) {
    const p1 = db.collection("Subscriptions").doc("subscriptionId").collection("Payment").doc(paymentId).update({
...
exports.setPaymentStatus = function(orderId, id) {
    ...
    const p1 = db.collection("Subscriptions").doc(orgDomain).collection("Payment").doc(paymentId).get().then((doc)=>{
        data = doc.data();
    ...
    Promise.resolve(p1).then(()=>{
        const promise = db.collection("Subscriptions").doc(orgDomain).collection("Payment").doc(paymentId).update({
